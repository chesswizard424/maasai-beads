...
PRODUCTS_FILE = Path("js/products.js")
IMAGE_ROOT = Path("images/processed/new-catalogue-116")
...
allocation = {
    ...
    20: ("MB-OTH-007", "Mixed Colour Beadwork Set"),
    ...
    69: ("MB-OTH-008", "Multicolour Beaded Strands"),
    ...
    83: ("MB-OUT-005", "Multicolour Beaded Outfit"),
    ...
    98: ("MB-OTH-009", "Rainbow Strand Collection"),
    99: ("MB-NK-048", "Blue Spiral Medallion"),
    100: ("MB-BELT-026", "Triple Heritage Tassels"),
    101: ("MB-OTH-010", "Golden Maasai Mosaic"),
    ...
    109: ("MB-OTH-011", "White Pearl Strands"),
...
if len(allocation) != 116:
    raise SystemExit(f"ERROR: Expected 116 new products, found {len(allocation)}")

new_ids = {pid for pid, _ in allocation.values()}
conflicts = sorted(existing_ids & new_ids)

if conflicts:
    print("ERROR: These IDs already exist:")
...
for number, (pid, name) in allocation.items():
    prefix = pid.rsplit("-", 1)[0]
    category = prefix_map[prefix]
    folder = folder_map[category]

    image = IMAGE_ROOT / folder / f"{number:03d}.png"
...
if last == -1:
    raise SystemExit("ERROR: Could not find final }; in products.js")

new_text = text[:last] + insert + "\n" + text[last:]

PRODUCTS_FILE.write_text(new_text)

final_ids = set(re.findall(r'"(MB-[A-Z0-9-]+)"\s*:', new_text))

print()
print("SUCCESS")
print("Original products: 89")
print("New products: 116")
print(f"Total products: {len(final_ids)}")
